...
define([
	...
	'underscore',
	'bootstrap',
	'text!src/tpl/homepage/category.tpl',
	'text!src/tpl/homepage/category-list.tpl',
	'text!src/tpl/homepage/category-detail.tpl',
	'text!src/tpl/homepage/preview-category.tpl',
	'src/util',
	'fileupload',
	...
	var view = Backbone.View.extend({
		...
		startQuery: function(){
			...
			$.ajax({
				loader: true,
				url: '../static/js/src/data/category-list.json',
				dataType: "json",
		...
		saveSlogan: function(){
			var form = this.$el.find(".slogan-form");
			var hint = form.find(".slogan-hint-block");
			var slogantitle = form.find("input[name='title']").val();
			var slogandescription = form.find("input[name='description']").val();
			if(slogantitle == ""){
				hint.text("Slogan标题不能为空！");
				setTimeout(function(){
					hint.text("");
				}, 2000);
				return false;
			}
			if(slogandescription == ""){
				hint.text("Slogan介绍不能为空！");
				setTimeout(function(){
					hint.text("");
				}, 2000);
				return false;
			...
			var view = this;
			var formValues = $(".h-category-view .slogan-form").serialize();
		...
		confirmDelete: function(e){
			$("#delete-modal").find(".confirm-delete-btn").attr("data-category-id", $(e.currentTarget).attr("data-category-id"));
		...
		previewModule: function(){
			...
			$.ajax({
				loader: true,
				url: '../static/js/src/data/preview-category.json',
				dataType: "json",
				success: function (data, status) {
					if (data.statusCode) {
						panel.html(tpl(data));
		...
		createCategory: function(){
			...
			$.ajax({
				loader: true,
				url: '../static/js/src/data/category-new.json',
				dataType: "json",
				success: function (data, status) {
					if (data.statusCode) {
		...
		editCategory: function(e){
			...
			$.ajax({
				loader: true,
				url: '../static/js/src/data/category-detail.json',
				data: {
					id: categoryId
				},
				dataType: "json",
		...
		renderResults: function(data) {
			this.$el.find(".h-category-view .category-content").html(this.listTemplate(data));
		}
	});
	return view;
});