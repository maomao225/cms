...
define([
	...
	var view = Backbone.View.extend({
		...
		events: {
			...
			'click .confirm-prohibit-btn': 'confirmProhibit',
			'click .prohibit-btn': 'prohibitAll',
		...
		refreshList: function() {
			var view = this;
			var formValues = $(".user-view .q-form").serialize() + "&recordsPerPage=" + view.pageInfo.recordsPerPage + "&page=" + view.pageInfo.page;
		...
		confirmProhibit: function(e){
			var target = $(e.currentTarget);
			var prohibitStatus = (target.text() == "开启" ? "1" : "0");
			var modal = $("#prohibit-modal");
			modal.find(".prohibit-status").text(target.text());
			modal.find(".prohibit-btn").data("active-status", prohibitStatus);
			modal.modal("show");
		},
		prohibitAll: function(e){
			var view = this;
			var target = $(e.currentTarget);
			var prohibitStatus = target.data("active-status");
			$.ajax({
				loader: true,
				url: '../static/js/src/data/user-list.json',
				data: {
					is_active: prohibitStatus
				},
				dataType: "json",
				success: function (data, status) {
					if (!data.statusCode) {
						$("#prohibit-modal").modal("hide");
						view.$el.find(".q-form .confirm-prohibit-btn").text(prohibitStatus=="1" ? "关闭" : "开启");
						view.startQuery();
					}else{
			...
			$.ajax({
				...
				success: function (data, status) {
					if (data.statusCode > 0) {
						view.$el.find(".prohibit-block").show();
						view.$el.find(".confirm-prohibit-btn").text(data.is_active == "1" ? "关闭" : "开启");
					}
				},
				error: function (xhr, errorType, error) {
					console.log(error);
