 ...
 *===============================================================================
 * 一、功能说明：
 * 1. 搜索方式：从 data.list 的所有字段数据中查询 keyword 的出现，或字段数据包含于 keyword 中
    ...
    var methods = {
        init: function(opts){
            ...
            options = $.extend({
                ...
                inputWarnColor: "rgba(255,0,0,0)", //输入框内容不是下拉列表选择时的警告色
                listStyle: {
            ...
            */
            ...
            function getData(keyword, $input, callback, opts) {
                var data, validData, filterData = {list:[]}, i, obj, hyphen, URL, len;
                ...
                if (opts.url) {
                    ...
                    if (validData) { //输入不为空时则进行匹配
                        if (!keyword) {
                            ...
                            for (i = 0; i < len; i++) {
                                for (obj in data.list[i]) {
                                    if (
                                        $.trim(data.list[i][obj]) &&
                                        (inSearchFields(obj, opts) || inEffectiveFields(obj, opts)) &&
                                        (data.list[i][obj].toString().indexOf(keyword) !== -1 || keyword.indexOf(data.list[i][obj]) !== -1)
                                    ){
                                        filterData.list.push(data.list[i]);
                                        break;
                                    }
                                }
                            }
            ...
            function refreshDropMenu($input, data, opts) {
                ...
                if (data === false || (len = data.list.length) === 0) {
                ...
                for (i = 0; i < len; i++) {
                    index = 0;
                    tr = "";
                    idValue = data.list[i][opts.idField] || "";
                    keyValue = data.list[i][opts.keyField] || "";

                    for (j in data.list[i]) {
                        //标记作为 value 和 作为 id 的值
                        if (!keyValue && opts.indexKey === index) {
                            keyValue = data.list[i][j];
                        }
                        if (!idValue && opts.indexId === index) {
                            idValue = data.list[i][j];
                        }
                        ...
                        tr +='<td data-name="' + j + '">' + data.list[i][j] + '</td>';
                    }
            ...
            function setValue($input, keywords, opts){
                ...
                if (opts && opts.multiWord) {//多关键字支持，只设置 val
                    ...
                    $input.attr("data-id", id).focus().val(label);
                }
